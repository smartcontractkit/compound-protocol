...
function printUsage() {
  console.log(`
usage: npx saddle script token:cpordelegator:deploy {tokenConfig}

note: pass VERIFY=true and ETHERSCAN_API_KEY=<api key> to verify contract on Etherscan

example:

npx saddle -n mainnet script token:cpordelegator:deploy '{
...
(async function() {
  ...
  console.log(`Deploying CPoRDelegator with ${JSON.stringify(conf)}`);
  ...
  let contract = await saddle.deploy('CPoRDelegator', deployArgs);
  ...
  if (env['VERIFY']) {
    ...
    await saddle.verify(etherscanApiKey, contract._address, 'CPoRDelegator', deployArgs, 0);
